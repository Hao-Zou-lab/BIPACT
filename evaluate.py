...
def calculate_clisi(embedding, cell_labels, k=50):
    nbrs = NearestNeighbors(n_neighbors=k).fit(embedding)
    _, indices = nbrs.kneighbors(embedding)
    
    clisi_scores = []
    for ind in indices:
        # Cell labels of k-nearest neighbors
        neighbor_labels = cell_labels[ind]
        # Frequency of the most common cell label
        max_freq = np.max(np.bincount(neighbor_labels))
        # Calculate cLISI score for each cell
        clisi_score = max_freq / k
        clisi_scores.append(clisi_score)
    
    return np.mean(clisi_scores)  # Return the average cLISI score

def calculate_kbet(embedding, batch_labels, k=50):
    nbrs = NearestNeighbors(n_neighbors=k).fit(embedding)
    _, indices = nbrs.kneighbors(embedding)
    batch_labels_array = np.array(batch_labels) - 1
  
    kbet_scores = []
    expected = np.bincount(batch_labels_array, minlength=2) / len(batch_labels_array)
    
    for i, ind in enumerate(indices):
        observed = np.bincount(batch_labels_array[ind], minlength=2)
        if np.sum(observed) > 0:
            observed = observed / np.sum(observed)
            chi2_stat, p_value = chisquare(f_obs=observed, f_exp=expected)
            kbet_scores.append(p_value)
        else:
            kbet_scores.append(1)  # Assume no batch effect if no neighbors

    average_kbet = np.mean(kbet_scores)
    return average_kbet

def evaluate_scores(div_ent_code, sil_code, cell_labels, dataset_labels, num_datasets, div_ent_dim, sil_dim, sil_dist, kn = 50):
    ...
    clisi_score = calculate_clisi(div_ent_code, cell_labels, k=kn)

    if sil_code.shape[1] > sil_dim:
        sil_code = PCA(n_components=sil_dim).fit_transform(sil_code)
    batch_asw_scores = silhouette_samples(sil_code, dataset_labels, metric=sil_dist)
    batch_asw = np.mean(batch_asw_scores)
    kbet_score = calculate_kbet(div_ent_code, dataset_labels, k=kn)
    ...
    return div_score, ent_score, sil_score, batch_asw, kbet_score, clisi_score