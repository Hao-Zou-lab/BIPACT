...
def intersect_dataset(dataset_list):
    ...
    """
    dataset_labels = ['sample_labels', 'cell_labels', 'cluster_labels', 'study_label']  # labels in a dataset
...
def pre_processing(dataset_file_list, pre_process_paras):
    ...
    for i, data_file in enumerate(dataset_file_list):
        dataset = read_csv(data_file, take_log)
        
        # Assign study_label
        num_samples = dataset['gene_exp'].shape[1]
        # Create one-hot encoded study_label
        study_label = np.zeros((num_samples, num_datasets))
        study_label[:, i] = 1
        dataset['study_label'] = study_label
    ...
    dataset_list = intersect_dataset(dataset_list)

    # After all datasets are read, convert study_label
    for dataset in dataset_list:
        study_label = dataset['study_label']
        # Convert [0,1] and [1,0] to [-0.5, 0.5] and [0.5, -0.5]
        study_label = study_label * 1.0  # Ensure it's float type
        study_label[study_label == 0] = -0.5
        study_label[study_label == 1] = 0.5
        dataset['study_label'] = study_label
    return dataset_list
